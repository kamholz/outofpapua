...
const allowed = new Set(['gloss', 'glosslang', 'headword', 'lang1', 'lang2', 'loose', 'page', 'pagesize']);
...
function makeQuery(q, { filterHeadwordGloss, lang, locals, query }) {
  q = q
    .from('entry')
...
export const GET = requireContributor(async ({ locals, url: { searchParams } }) => {
  ...
  const [lang1Set, lang1PlusSet] = await getLanguageIdsSet(query.lang1);
  const [lang2Set, lang2PlusSet] = await getLanguageIdsSet(query.lang2);
  if (!(lang1Set && lang2Set)) {
    throw error(500);
  }

  removePlusOverlap(lang1Set, lang2PlusSet);
  removePlusOverlap(lang2Set, lang1PlusSet);

  const lang1 = [...new Set([...lang1Set, ...lang1PlusSet])];
  const lang2 = [...new Set([...lang2Set, ...lang2PlusSet])];
  ...
  const subq = knex
    .with('lang1', (cte) => makeCte(cte, { filterHeadwordGloss: true, lang: lang1, locals, query }))
    .with('lang2', (cte) => makeCte(cte, { filterHeadwordGloss: false, lang: lang2, locals, query }))
    .from('lang1')
    .leftJoin('lang2', function () {
      this.on('lang2.language_id', 'lang1.language_id');
      if (query.loose) {
        this.on('lang2.txt_degr_fuzzy', 'lang1.txt_degr_fuzzy');
      } else {
  ...
  const rowCount = await getCountDistinct(makeQuery(knex, { filterHeadwordGloss: true, lang: lang1, locals, query }), 'entry.id');
...
function removePlusOverlap(lang1Set, lang2PlusSet) {
  for (const lang of lang1Set) {
    if (lang2PlusSet.has(lang)) {
      lang2PlusSet.delete(lang);
    }
  }
}
